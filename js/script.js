...
// Current email index for simulation
...
function showMission(missionNumber) {
    ...
    // Show target mission
    const targetMission = document.getElementById(`mission-${missionNumber < 6 ? '0' + missionNumber : 'final-module'}`);
    if (targetMission) {
...
function initializeEmailSimulation() {
    ...
    document.getElementById('trust-btn').addEventListener('click', () => handleEmailAction(true));
    document.getElementById('delete-btn').addEventListener('click', () => handleEmailAction(false));
}

function showNextEmail() {
    if (currentEmailIndex >= emailScenarios.length) {
        // All emails analyzed
        document.getElementById('mission-01-complete').classList.remove('hidden');
        return;
    }
...
function checkPasswordStrength() {
    ...
    if (metRequirements === 5) {
        ...
        document.getElementById('mission-02-complete').classList.remove('hidden');
    } else {
        vaultStatus.className = 'vault-status';
        vaultStatus.innerHTML = '';
        document.getElementById('mission-02-complete').classList.add('hidden');
    }
}
...
function showCurrentScenario() {
    if (currentScenario >= scenarios.length) {
        document.getElementById('mission-03-complete').classList.remove('hidden');
        return;
    }
...
function handleRiskClick(riskElement) {
    ...
    if (!foundRisks.has(riskType)) {
        ...
        if (risksFound === 4) {
            setTimeout(() => {
                document.getElementById('mission-04-complete').classList.remove('hidden');
...
function updateRiskCounter() {
    document.getElementById('risk-count').textContent = risksFound;
}
...
function handlePiiClick(piiElement) {
    if (!piiElement.classList.contains('redacted')) {
        ...
        if (piiItemsFound === totalPiiItems) {
            setTimeout(() => {
                document.getElementById('mission-05-complete').classList.remove('hidden');
...
function updatePiiCounter() {
    document.getElementById('pii-count').textContent = piiItemsFound;
}
...
window.resetTraining = resetTraining;// js/script.js

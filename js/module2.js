...
const module2Manager = {
    ...
    handleAction(dataset) {
        ...
        switch (action) {
            ...
            case 'start-training': if (window.digitalShieldProgress) { window.digitalShieldProgress.startModule(2); } this.showSection('training-phase-1'); this.updateProgress(2); break;
    ...
    renderStaticContent() { this.dom.passwordGrid.innerHTML = this.commonPasswords.map(p => `<span>${p}</span>`).join(''); },
    ...
    testExample(password) { this.dom.passwordTester.value = password; this.testPasswordStrength(); this.dom.passwordTester.focus(); },
    completeModule() { if (window.digitalShieldProgress) { const passedChallenges = this.challengeScores.filter(Boolean).length; const finalScore = Math.round((passedChallenges / 3) * 100); window.digitalShieldProgress.completeModule(2, finalScore); } alert('Module 2 complete! Progress saved. Returning to Mission Control.'); window.location.href = 'index.html'; }
};
